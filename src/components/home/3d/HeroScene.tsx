"use client";
import { Suspense } from "react";
import { Canvas } from "@react-three/fiber";
import { Environment, Stars, AdaptiveDpr } from "@react-three/drei";
import { TechOrb } from "./TechOrb";
import { FloatingRings } from "./FloatingRings";
import { ParticleField } from "./ParticleField";
import { GridFloor } from "./GridFloor";
interface HeroSceneProps {
  mouseX?: number;
  mouseY?: number;
  isMobile?: boolean;
}
function SceneContent({ mouseX = 0, mouseY = 0, isMobile = false }: HeroSceneProps) {
  return (
    <>
      {/* Lighting */}
      <ambientLight intensity={0.2} />
      <pointLight position={[5, 5, 5]} intensity={2} color="#3B82F6" />
      <pointLight position={[-5, -3, -5]} intensity={1.5} color="#7C3AED" />
      <pointLight position={[0, 3, -3]} intensity={1} color="#06B6D4" />
      {/* Main orb */}
      <TechOrb
        position={[0, 0, 0]}
        scale={1.2}
        speed={0.8}
        distort={0.5}
        color="#3B82F6"
      />
      {/* Orbiting orbs */}
      <TechOrb position={[2.5, 1, -1]} scale={0.3} speed={1.5} color="#7C3AED" />
      <TechOrb position={[-2, -0.5, 0.5]} scale={0.2} speed={2} color="#06B6D4" />
      {/* Rings */}
      <FloatingRings />
      {/* Particles */}
      <ParticleField count={isMobile ? 300 : 600} radius={5} mouse={{ x: mouseX, y: mouseY }} />
      {/* Background stars */}
      <Stars radius={100} depth={50} count={isMobile ? 1000 : 2000} factor={3} saturation={0} fade speed={0.5} />
      {/* Grid floor */}
      <GridFloor />
      {/* Environment for reflections */}
      <Environment preset="night" />
      {/* Performance optimization */}
      <AdaptiveDpr pixelated />
    </>
  );
}
export default function HeroScene({ mouseX = 0, mouseY = 0, isMobile = false }: HeroSceneProps) {
  return (
    <Canvas
      camera={{ position: [0, 0, 5] }}
      style={{ width: '100%', height: '100%', background: '#050505' }}
    >
      <ambientLight intensity={1} />
      <pointLight position={[10, 10, 10]} />
      <mesh>
        <sphereGeometry args={[1, 32, 32]} />
        <meshStandardMaterial color="#3B82F6" emissive="#3B82F6" emissiveIntensity={0.5} />
      </mesh>
    </Canvas>
  );
}
