...
const getImageData = (url: string, imageProps: WixImageDataProps): ImageData | undefined => {
  ...
  } else if (url.startsWith('https://static.wixstatic.com/')) {
...
const WixImage = forwardRef<HTMLImageElement, WixImageProps>(
  ({ data, fittingType = 'fill', className, style, ...props }, parentRef) => {
    ...
    // Degrade gracefully without Wix image transformations by using the static media URL directly.
    if (size) {
      ...
      const query = new URLSearchParams({
        originWidth: String(data.width),
        originHeight: String(data.height),
        width: String(Math.round(targetWidth)),
        height: String(Math.round(targetHeight)),
      });
      if (focalPoint) {
        query.set('focalPointX', String(focalPoint.x));
        query.set('focalPointY', String(focalPoint.y));
      }
      imgProps.src = `https://static.wixstatic.com/media/${data.id}?${query.toString()}`
    } else {
      imgProps.src = `https://static.wixstatic.com/media/${data.id}?originWidth=${data.width}&originHeight=${data.height}`
      imgProps['data-placeholder-image'] = false
    }
