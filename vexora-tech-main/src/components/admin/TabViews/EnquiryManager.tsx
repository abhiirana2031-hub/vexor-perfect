                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-secondary flex items-center gap-2">
                        <span className="w-1 h-1 rounded-full bg-secondary animate-pulse shadow-neon-cyan" />
                        Source Node
                      </h4>
                      {currentStatus !== 'pending' && (
                        <span className={`text-[8px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest relative overflow-hidden group/badge ${
                          currentStatus === 'deleted' ? 'bg-destructive/10 border border-destructive/20 text-destructive shadow-[0_0_10px_rgba(239,68,68,0.2)]' : 'bg-neon-green/10 border border-neon-green/20 text-neon-green shadow-[0_0_10px_rgba(57,255,20,0.2)]'
                        }`}>
                          <span className={`absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover/badge:animate-[shimmer_1.5s_infinite]`} />
                          {currentStatus}
                        </span>
                      )}
                    </div>
